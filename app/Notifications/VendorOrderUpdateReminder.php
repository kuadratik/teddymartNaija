...
class VendorOrderUpdateReminder extends Notification implements ShouldQueue
{
    ...
    public function toMail($notifiable): MailMessage
    {
        return (new MailMessage)
            ...
            ->line("We hope this email finds you well. We're following up regarding Order {$this->order->order_number}. As per our records, the order was shipped on {$this->order->shipped_at->format('Y-m-d')}, but we noticed that the status has not yet been updated in our system.")
            ...
            ->line("Shipping Date: {$this->order->shipped_at->format('Y-m-d')}")
