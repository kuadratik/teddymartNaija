...
export const extractHashCode = (input: string): string => {
  if (!input) return 'N/A'

  // Try to match the standard format with 4 hex digits
  const match = input.match(/-([a-f0-9]{4,})-/)
  if (match) return `#${match[1]}`

  // Alternative: look for any segment of 4+ alphanumeric characters after a hyphen
  const altMatch = input.match(/-([a-zA-Z0-9]{4,})/)
  if (altMatch) return `#${altMatch[1]}`

  // If no pattern matches, return a truncated version of the order number
  return input.length > 8 ? `#${input.substring(input.length - 6)}` : `#${input}`
}
...
export const isHolidayPeriod = (): boolean => {
  ...
  // Create date range for current holiday season
  const seasonStart = dayjs(`${currentYear}-12-17`)
  const seasonEnd = dayjs(`${currentYear + 1}-01-18`)

  // Handle the case when we're in January
  if (today.month() === 0) {
    // January is 0
    const lastYearStart = dayjs(`${currentYear - 1}-12-18`)
    const thisYearEnd = dayjs(`${currentYear}-01-18`)

    return today.isBetween(lastYearStart, thisYearEnd, 'day', '[]') // Include start and end dates
  }

  // For all other months
  return today.isBetween(seasonStart, seasonEnd, 'day', '[]') // Include start and end dates
}
...
// 'DD/MM/YYYY . hh:mm A'
export const newUserTimeZoneFormatDate = (isoString: string, format: string) => {
...
export const newFormatPhoneNumberByCountry = (
  phoneNumber: string,
  defaultCountry?: CountryCode,
  formatType: 'international' | 'national' | 'e164' = 'international'
): string => {
  if (!phoneNumber) return ''

  try {
    // Try to parse with or without default country
    const parsedNumber = parsePhoneNumberFromString(phoneNumber, defaultCountry)

    // If parsing succeeded, format according to requested type
    if (parsedNumber) {
      if (formatType === 'international') return parsedNumber.formatInternational()
      if (formatType === 'national') return parsedNumber.formatNational()
      if (formatType === 'e164') return parsedNumber.format('E.164')

      // Fallback to international format
      return parsedNumber.formatInternational()
    }

    // If parsing failed but we have a default country, try to format as incomplete
    if (defaultCountry) {
      return new AsYouType(defaultCountry).input(phoneNumber)
    }
