...
import {useCreateStorePromotionMutation, useGetPromotionsPlansQuery} from '@/services/advertisement'
...
const PromoteStorePage = () => {
  ...
  const storeListOptions = [
    ...(userStoreData?.data || []).map((store: IUserStoreDatum) => ({
      label: store.name,
      value: store.id
    }))
  ]
  ...
  const getStoreById = (id: number) => {
    return (userStoreData?.data || []).find((store: IUserStoreDatum) => store.id === id)
  }
