...
const Vendor = () => {
  ...
  const {data: listingTotalData} = useGetStoreMetricsQuery({params: type, userStore: isActiveUser?.slug})
  ...
  return (
    <>
      <SEOHead
        title={`AfricanDiasporaMart | Vendor ${capitalizeFirstLetter(type)} `}
          ...
          <div className="flex w-full items-center justify-center py-[40px]">
            <div className="flex w-full flex-col items-center justify-center gap-[21px] rounded-[37px] bg-black py-[24px]">
              ...
              <div className="flex flex-col items-center justify-center">
                {isAuthenticatedUser?.store?.length > 1 ? (
                  <>
                    {' '}
                    <div className="flex w-full cursor-pointer items-center justify-center px-6 py-3">
                      <Dropdown
                        overlay={
                          <Menu className="flex flex-col gap-1">
                            {isAuthenticatedUser?.store.map((option: any, index: any) => (
                              <div
                                key={option.id}
                                onClick={() => {
                                  handleStoreSwitch(option)
                                }}
                                className={`flex cursor-pointer items-center justify-between gap-2 p-4 hover:bg-gray-200 ${isActiveUser?.name == option.name ? 'bg-gray-200' : ''}`}
                              >
                                {' '}
                                <p className="text-[14px] font-semibold text-[#1D1D1D] visited:text-[#27104E]">
                                  {option.name}
                                </p>
                      ...
                      >
                        <div className="flex h-[24px] flex-row items-center gap-2 font-inter text-sm font-medium text-[#33357D]">
                          ...
                          <Icon icon={'ep:arrow-down-bold'} className="text-[26px] text-white" />
                        </div>
                      </Dropdown>
                    </div>
                  </>
