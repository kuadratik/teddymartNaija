...
const CategoryPage = () => {
  ...
  return (
    <>
      <SEOHead
        title={`AfricanDiasporaMart | ${capitalizeOnlyFirstLetter(type)} Category`}
                    ...
                    <div className="flex w-full items-center gap-[30px] border-b border-[#E4E4E4] py-2" key={index}>
                      ...
                      <div className="flex w-full flex-col gap-1">
                        ...
                        <CustomButton
                          ...
                          onClick={() => {
                            if (type === 'product') {
                              router.push(`/store/${item.store?.slug}?type=${type}`)
                            } else {
                              router.push(`/store/details/${item.store?.slug}?slug=${item?.slug}&type=${type}`)
                            }
                          }}
                        >
