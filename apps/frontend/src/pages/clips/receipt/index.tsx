...
import {useGetUserOrderDetailsQuery} from '@/services/auth/clips'
...
const InvoiceView = () => {
  ...
  const {
    ...
    {
      order_id: router?.query?.order_id ? router.query.order_id.toString() : ''
    },
    {
      skip: !router?.query?.order_id
    }
  )
  console.log('🚀 ~ InvoiceView ~ userOrderDetails:', userOrderDetails)
  ...
  // Function to merge weight data from stored clips with order data
  const getOrderDetailsWithWeight = (orders: any[]) => {
    ...
    return orders.map(order => ({
      ...order,
      order_details: order.order_details?.map((item: any) => {
        var _item_listing
        // First check if weight is already in the API response
        if (
          item === null || item === void 0
            ? void 0
            : (_item_listing = item.listing) === null || _item_listing === void 0
              ? void 0
              : _item_listing.weight
        ) {
          return item
        }
        ...
        if (weightData === null || weightData === void 0 ? void 0 : weightData.weight) {
          return {
            ...item,
            listing: {
              ...item.listing,
  ...
  // Process data from either source into a consistent format
  useEffect(() => {
    if (router?.query?.order_id && isOrderDetailsSuccess && userOrderDetails) {
      setDataSource('api')
      // new API returns an array in data
      setOrderData((userOrderDetails as any).data || [])
    } else if (paymentSuccessCheckoutData) {
      setDataSource('localStorage')
      // Merge weight data from localStorage for receipt display
      const mergedData = getOrderDetailsWithWeight(paymentSuccessCheckoutData)
      setOrderData(mergedData)
    }
  }, [router?.query?.order_id, userOrderDetails, isOrderDetailsSuccess, paymentSuccessCheckoutData])

  // Get shipping address from API data or localStorage
  const shippingAddress = useMemo(() => {
    if (dataSource === 'api' && orderData?.[0]?.shipping_address) {
      return orderData[0].shipping_address
    }
    return selectedAddress
  }, [dataSource, orderData, selectedAddress])

  const data = orderData?.[0] || null

  // Calculate combined order values
  const combinedOrders = orderData?.reduce(
  ...
  if (isLoading && router?.query?.order_id) {
    return (
        ...
        <div className="mx-auto max-w-5xl">
          <div id="invoice-order" className={isLoadingInvoice ? 'blur-md' : ''}>
            <div className="flex justify-between border-b-[1.5px] border-dashed p-3 py-8 pb-10">
              <div>
                <Image src={'/assets/dark_logo.svg'} className="object-contain" alt="logo" width={140} height={19} />
              ...
              {isDesktop && (
                <div className="container mx-auto py-4">
                  <table className="w-full border-collapse">
                    ...
                    <tbody>
                      {orderData &&
                        orderData.map((product: IPaymentSuccessCheckoutDataTopLevel) => (
                          <>
