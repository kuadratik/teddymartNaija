...
import {capitalizeOnlyFirstLetter, isIOS} from '@/utils/fx'
...
const ShippingPage = () => {
  ...
  const handleSelectedAddress = async () => {
    ...
    // Store clip weight data before payment for later retrieval
    if (clipProductInfo && clipProductInfo.length > 0) {
      const clipWeightData = clipProductInfo.map((clip: any) => ({
        listing_id: clip.listing_id,
        weight: clip.weight,
        name: clip.name,
        store_id: clip.store_id
      }))
      localStorage.setItem('clipWeightData', JSON.stringify(clipWeightData))
    }
    ...
    let payload = {
      ...
      return_url:
        selectedPaymentGateway === 'paypal'
          ? `${window.location.origin}/customer?tab=order-history`
          : `${window.location.origin}/clips/payment/successful`,
  ...
  const handleButtonClick = async () => {
    const url = await handleSelectedAddress()
    if (url) {
  ...
  return (
    <>
      <SEOHead
        title={`AfricanDiasporaMart | Shipping`}
                    ...
                    <div className="flex flex-col justify-between gap-8 md:justify-normal lg:mx-auto lg:w-full lg:max-w-7xl lg:gap-[72px]">
                      ...
                      <div className="w-full lg:flex lg:gap-[56px]">
                                                            ...
                                                            <div className="flex w-full items-center justify-between gap-2">
                                                              <div className="">
                                                                ...
                                                                {method.pick_up_time ? (
                                                                  ...
                                                                  <p className="text-sm text-gray-500">
                                                                    Shipping duration: {method.duration_number}{' '}
                                                                    {method.duration_type}
                                                                    {method.duration_number > 1 ? 's' : ''}
                                                                  </p>
                                                                )}
                                                              </div>
                                                              {method.pick_up_time && (
                        ...
                        <YourOrderComponent
                          ...
                          btnText={'Pay Now'}
                          handleSelectedAddress={handleButtonClick}
                        />
                      </div>
                    </div>
                  ) : (
