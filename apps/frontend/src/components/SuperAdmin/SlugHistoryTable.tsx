...
const SlugHistoryTable: React.FC<SlugHistoryTableProps> = ({ searchQuery, currentPage = 1, onPageChange, dateRange }) => {
  ...
  const saveNote = async (entryId: number, noteText?: string) => {
    try {
      ...
      // Close dropdown
      setDropdownOpen(prev => ({ ...prev, [`${entryId}-desktop`]: false }))
  ...
  const renderStatusSelect = (entry: SlugHistoryEntry) => {
    const dropdownKey = `${entry.id}-desktop`
    ...
    <div className="flex flex-col gap-6">
      ...
      {brandGroups.map((brandGroup: BrandSlugHistory) => (
        ...
        >
          ...
          >
            <TextComponent as="h2" className="md:text-[20px] self-end font-semibold text-gray-900">
        ...
        <div className="flex flex-col gap-4 border-t border-gray-200 pt-6 sm:flex-row sm:items-center sm:justify-between sm:gap-0">
          ...
          <div className="flex items-center justify-center gap-2 sm:justify-end">
            ...
            <div className="flex w-full items-center justify-between gap-2 sm:hidden">
              ...
              <div className="flex-1 text-center">
                <span className="inline-flex min-w-[60px] items-center justify-center rounded-lg bg-black px-3 py-2 text-[13px] font-medium text-white">
                  {currentPage} of {allHistoryData.data.last_page}
                </span>
              </div>
            ...
            <div className="hidden items-center gap-2 sm:flex">
              ...
              {/* Next button */}
              <button
                onClick={() => onPageChange?.(allHistoryData.data.last_page)}
                disabled={currentPage === allHistoryData.data.last_page}
                className="flex h-9 w-9 items-center justify-center rounded-lg border border-gray-200 bg-white transition-colors hover:bg-gray-50 disabled:opacity-50"
              >
                <Icon icon="heroicons:chevron-right" className="text-lg text-gray-600" />
