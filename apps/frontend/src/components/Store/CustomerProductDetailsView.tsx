...
const CustomerProductDetailsView = ({
  ...
  console.log('🚀 ~ currVariantDetails:', currVariantDetails)

  // Add error handling for JSON parsing
  let size_chart_html = null
  try {
    const rawContent = data?.attributes?.size_chart_html
    if (rawContent) {
      // Check if content appears to be HTML
      if (
        typeof rawContent === 'string' &&
        (rawContent.trim().startsWith('<') || rawContent.includes('</') || rawContent.includes('>'))
      ) {
        // It's HTML content - don't parse as JSON
        size_chart_html = {isHtml: true, content: rawContent}
      } else {
        // Try parsing as JSON
        size_chart_html = JSON.parse(rawContent)
      }
    }
  } catch (error) {
  ...
  const removedTitleSizeChart =
    size_chart_html && !size_chart_html.isHtml
      ? size_chart_html?.columns?.filter((column: {dataIndex: string}) => column.dataIndex !== 'operation')
      : []
    ...
    <React.Fragment>
      <div className="w-full">
        <div className="flex w-full justify-between">
          <div className="w-full">
              ...
              {customer ? (
                <TextComponent as="p" className="text-[14px] text-[#6B7280]">
                  ...
                  <Link
                    ...
                    className="cursor-pointer text-black underline hover:text-black hover:underline hover:opacity-80"
                  >
                    {data?.store?.name}
                  </Link>{' '}
                  store
            ...
            {removedTitleSizeChart.length > 0 ||
            (data?.attributes?.size_chart_image && data?.attributes?.size_chart_image !== 'teddymart/images/') ? (
              <Button
                onClick={() => {
        ...
        <div className="mt-[20px] flex w-full flex-col gap-4 md:flex-row md:items-center md:justify-between">
          {(currVariant !== null
            ? currVariantDetails?.size?.length > 0 && !currVariantDetails?.size?.includes?.('[]')
            : data?.attributes?.size &&
              safeJsonParse(data?.attributes?.size)?.length > 0 &&
              !data?.attributes?.size?.includes?.('[]')) && (
            <div className="flex flex-col justify-between gap-6 md:flex-row md:gap-0">
              <div>
                ...
                {currVariant !== null &&
                currVariantDetails?.size?.length &&
                !currVariantDetails?.size?.includes?.('[]') ? (
      ...
      {showDeleteProduct && (
        ...
        >
          ...
          {size_chart_html ? (
            size_chart_html.isHtml ? (
              <div dangerouslySetInnerHTML={{__html: size_chart_html.content}} className="size-chart-html-container" />
            ) : (
              <Table<DataType>
                bordered
                pagination={false}
