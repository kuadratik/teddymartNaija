...
const CountryStateFilter = ({
  ...
  const stateQueryArg = selectedCountry?.id
    ? {
        search: '',
  ...
  const {data: states = [], isLoading: isStatesLoading} = useGetStateQuery(stateQueryArg)
    ...
    <div className="relative" ref={dropdownRef}>
      ...
      >
        <div className="relative">
          ...
          {isCountryOpen && (
            <div className="absolute right-0 top-16 z-40 flex max-h-[300px] min-w-[160px] flex-col gap-1 overflow-y-auto rounded-md border border-[#DFDFDF] bg-white p-1 shadow-f2 lg:w-full">
              <button
                onClick={() => {
