...
function EmptyData({btnTitle, title, btnAction, textNode}: IProps) {
  return (
    <div className="mt-4 rounded-md px-5 text-center">
      <div className="my-1 flex items-center justify-center bg-transparent">
        <Image className="!bg-transparent" src={'/assets/EmptyState.svg'} alt="noresuts" width={250} height={200} />
      </div>
      {textNode ? (
        textNode
      ) : (
