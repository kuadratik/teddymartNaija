...
import {capitalizeOnlyFirstLetter, newFormatPhoneNumberByCountry} from '@/utils/fx'
      ...
      <TextComponent as="p" className="mt-4 text-[14px] font-normal text-black">
        <span
          ...
          dangerouslySetInnerHTML={{
            __html: capitalizeOnlyFirstLetter(adsDataVal?.description)
          }}
        />
      </TextComponent>
        ...
        <div onClick={() => setShowPhoneNumber(!showPhoneNumber)} className="flex cursor-pointer items-center gap-6">
          <p className={'text-[14px] font-[500] text-black'}>
            {!showPhoneNumber ? (
              ...
              <>{newFormatPhoneNumberByCountry(`+${adsDataVal?.phone_number ?? ''}`)}</>
            )}
          </p>
