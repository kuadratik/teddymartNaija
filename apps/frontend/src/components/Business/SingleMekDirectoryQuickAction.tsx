...
const SingleMekDirectoryQuickAction = ({handleDownload, handleCopyLink, data}: IProps) => {
  ...
  return (
    <div
      className={`mx-5 grid grid-cols-2 gap-2 rounded-[11px] bg-white px-[30px] py-2 shadow-f1 lg:gap-6 ${
        !isHideAction ? 'md:grid-cols-3 lg:grid-cols-7' : 'md:grid-cols-2 lg:grid-cols-5'
      }`}
    >
      {/* copy link */}
      <button
