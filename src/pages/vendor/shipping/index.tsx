...
// Helper function to pluralize duration type based on number
const formatDurationType = (type: string, number: number | null): string => {
  if (!type) return ''

  // Find the type in our durationTypes array
  const durationType = durationTypes.find(dt => dt.value === type)
  if (!durationType) return type

  // Add 's' if number is greater than 1
  return number && number > 1 ? `${durationType.label}s` : durationType.label
}
...
const Shipping = () => {
  ...
  >([{id: 1, value: '', amount: '', duration_number: null, duration_type: 'day'}])
  ...
  // Fez Delivery state
  const [fezDeliveryLocation, setFezDeliveryLocation] = useState<string>('')
  ...
  const resetFields = () => {
    setLocations([{id: 'my1', value: '', amount: '', duration_number: null, duration_type: 'day'}])
    setPickupTime('')
    setPickupStoreLocation('')
    setPickupId(null)
    setFezDeliveryLocation('')
  }
            ...
            if (vendorLocations && vendorLocations.length > 0) {
              setLocations(
                vendorLocations.map((loc: any, idx: any) => ({
                  ...
                  value: loc.location || '',
                  amount: loc.amount || '',
                  duration_number: loc.duration_number ? Number(loc.duration_number) : null,
                  duration_type: loc.duration_type || 'day'
                }))
              )
            }
          }
  ...
  const addLocation = () => {
    setLocations([
      ...locations,
      {id: `my${locations.length + 1}`, value: '', amount: '', duration_number: null, duration_type: 'day'}
    ])
  }
  ...
  const handleSave = async (methodType: string, loc: any) => {
    // Reset field errors for this location
    const locationId = loc?.id || 'pickup'
    setFieldErrors(prev => ({...prev, [locationId]: {}}))

    // Validate vendor-fulfilled shipping fields
    if (methodType === 'vendor-fulfilled shipping') {
      ...
      if (loc.duration_number === null || loc.duration_number === '') {
        errors.duration_number = 'Duration number is required'
      }

      if (!loc.duration_type) {
        errors.duration_type = 'Duration type is required'
      }

      // If there are errors, set them and show toast
      if (Object.keys(errors).length > 0) {
        setFieldErrors(prev => ({...prev, [loc.id]: errors}))
    ...
    const fulfilledPayload = {
      ...
      duration_number: loc?.duration_number,
      duration_type: loc?.duration_type
    }
          ...
          <div className="mx-auto w-full rounded-[12px] bg-white md:p-4">
            <Form className="flex w-full flex-col gap-6" onFinish={handleSubmit} layout="vertical">
              ...
              <div className="mb-4 border border-[#EAECEF] bg-[#FCFCFC] px-4 py-2">
                ...
                {selectedShippingMethod.length > 0 && (
                  <div className="mt-2 flex flex-row flex-wrap">
                    {selectedShippingMethod.map((method, idx) => (
                      <SelectedOptions
                        key={method.id}
                        text={method.name}
                        width="w-fit"
                        ...
                        onRemove={() => {
                          // For saved methods from backend, show delete modal
                          if (savedShippingMethod.find(saved => saved.name === method.name)) {
                            setCurrentMethod(method.name)
                            setShowDeleteMethodModal(true)
                          } else {
                            // For newly selected methods (not saved yet), just remove from selection
                            setSelectedShippingMethod(prev => prev.filter(m => m.name !== method.name))
                          }
                        }}
                      />
                    ))}
              ...
              {selectedShippingMethod.find(method => method.name === 'vendor-fulfilled shipping') && (
                <div className="mb-8 border border-[#EAECEF] bg-[#FCFCFC] px-4 py-4">
                  ...
                  {locations.map((loc, index) => (
                    <div key={loc.id} className="mb-4 flex flex-col gap-2 md:flex-row md:items-start md:gap-3">
                      <div className="flex-1">
                        ...
                        <Input
                          ...
                          onChange={e => {
                            ...
                            if (fieldErrors[loc.id]?.location) {
                              setFieldErrors(prev => ({
                                ...prev,
                                [loc.id]: {...prev[loc.id], location: ''}
                              }))
                            }
                          }}
                          ...
                          type={'text'}
                          value={loc.value}
                          status={fieldErrors[loc.id]?.location ? 'error' : ''}
                          classNames={{
                            input: `w-full rounded-[8px] border ${
                              fieldErrors[loc.id]?.location ? 'border-red-600' : 'border-gray-300'
                            } bg-[#F9FAFB] px-[15px] py-[11px] text-[#141414] shadow-f1 placeholder:font-medium hover:border-[#000000] focus:outline-none focus:ring-1 focus:ring-[#000000]`
                          }}
                        />
                        {fieldErrors[loc.id]?.location && (
                          <p className="mt-1 text-xs text-red-500">{fieldErrors[loc.id].location}</p>
                        )}
                      </div>
                      ...
                      <div className="flex-1">
                        ...
                        <Input
                          ...
                          type="text"
                          value={loc.amount}
                          status={fieldErrors[loc.id]?.amount ? 'error' : ''}
                          onChange={e => {
                            handleAmountChange(loc.id, formatWithCommas(e.target.value))
                            // Clear error when user types
                            if (fieldErrors[loc.id]?.amount) {
                              setFieldErrors(prev => ({
                                ...prev,
                                [loc.id]: {...prev[loc.id], amount: ''}
                              }))
                            }
                          }}
                          classNames={{
                            input: `w-full rounded-[8px] border ${
                              fieldErrors[loc.id]?.amount ? 'border-red-600' : 'border-gray-300'
                            } bg-[#F9FAFB] px-[15px] py-[11px] text-[#141414] shadow-f1 placeholder:font-medium hover:border-[#000000] focus:outline-none focus:ring-1 focus:ring-[#000000]`
                          }}
                        />
                        {fieldErrors[loc.id]?.amount && (
                          <p className="mt-1 text-xs text-red-500">{fieldErrors[loc.id].amount}</p>
                        )}
                      </div>

                      <div className="flex-1">
                        <p className="text-xm font-semibold text-black">Duration*</p>
                          ...
                          <div className="flex items-center gap-2">
                            <div className="w-1/3">
                              <TextInput
                                ...
                                onChange={e => {
                                  const newLocations = [...locations]
                                  newLocations[index].duration_number =
                                    e.target.value === '' ? null : Number(e.target.value)
                                  setLocations(newLocations)
                                  // Clear error when user types
                                  if (fieldErrors[loc.id]?.duration_number) {
                                    setFieldErrors(prev => ({
                                      ...prev,
                                      [loc.id]: {...prev[loc.id], duration_number: ''}
                                    }))
                                  }
                                }}
                                name={'duration_number'}
                                type={'number'}
                                value={loc.duration_number === null ? '' : loc.duration_number}
                                className={`w-full ${fieldErrors[loc.id]?.duration_number ? 'border-red-500' : ''}`}
                              />
                            </div>
                            <div className="w-2/3">
                              <SelectInput
                                ...
                                onChange={value => {
                                  const newLocations = [...locations]
                                  newLocations[index].duration_type = value
                                  setLocations(newLocations)
                                  // Clear error when user selects
                                  if (fieldErrors[loc.id]?.duration_type) {
                                    setFieldErrors(prev => ({
                                      ...prev,
                                      [loc.id]: {...prev[loc.id], duration_type: ''}
                                    }))
                                  }
                                }}
                                value={loc.duration_type}
                                className={`border py-1 ${fieldErrors[loc.id]?.duration_type ? 'border-red-500' : ''}`}
                                placeholder="Select duration"
                                renderLabel={item => formatDurationType(item.value, loc.duration_number)}
                              />
                              {fieldErrors[loc.id]?.duration_type && (
                                <p className="mt-1 text-xs text-red-500">{fieldErrors[loc.id].duration_type}</p>
                              )}
                            </div>
                          </div>
                          {fieldErrors[loc.id]?.duration_number && (
                            <p className="mt-1 text-xs text-red-500">{fieldErrors[loc.id].duration_number}</p>
                          )}
                        </div>
                      </div>

                      <div className="flex min-w-[100px] items-center justify-end gap-3 md:justify-start md:pt-1">
                        <Tooltip
                          placement="bottom"
                          title={'Save'}
                          color="#fff"
                        ...
                        locations.filter(item => typeof item.id === 'number').length > 1 ? (
                        ...
                        ) : locations.length > 1 && index !== 0 ? ( // Check if there is more than one location before rendering the remove button
                          <Tooltip
                            placement="bottom"
                            title={'remove'}
                            color="#fff"
                          ...
                          >
                            <div
                              className="flex h-[35px] w-[35px] cursor-pointer items-center justify-center rounded-md bg-[#FFEEEE] md:mt-6"
                              onClick={() => {
                                // Filter out the current location from the array
                                const newLocations = locations.filter(item => item.id !== loc.id)
                                setLocations(newLocations)
                              }}
                            >
                              <Icon icon="carbon:close-filled" className="text-[16px] text-red-600" />
                            </div>
                          </Tooltip>
                        ) : null}
                      </div>
              ...
              {/* Fez Delivery Section */}
              {selectedShippingMethod.find(method => method.name === 'Fez Delivery') && (
                <div className="mb-4 border border-[#EAECEF] bg-[#FCFCFC] px-4 py-4">
                  <h3 className="text-[20px] font-semibold">Fez Delivery</h3>
                  <p className="mb-4 text-sm font-normal text-[#6B7280]">
                    Complete and create shipping cost for all your destinations
                  </p>

                  <div className="flex flex-col gap-4">
                    <div className="flex-1">
                      <p className="mb-2 text-sm font-semibold text-black">Location*</p>
                      <Input.TextArea
                        placeholder="270 Jubilee Terrace, Fort Confederateworth 80336-4005"
                        onChange={e => {
                          setFezDeliveryLocation(e.target.value)
                        }}
                        name="fez_delivery_location"
                        value={fezDeliveryLocation}
                        rows={4}
                        className="max-h-[110px] min-h-[55px] w-full rounded-[8px] border border-gray-300 bg-[#F9FAFB] px-[15px] py-[11px] text-[#141414] shadow-f1 placeholder:text-sm placeholder:font-normal hover:border-[#000000] focus:outline-none focus:ring-1 focus:ring-[#000000]"
                      />
                    </div>
                    ...
                    <div className="flex justify-end">
                      ...
                      >
                        <div
                            ...
                            showPlannerToast({
                              options: {
                                customToast: (
                                  <CustomToast
                                    altText=""
                                    title={<>Fez Delivery location will be saved here</>}
                                    textColor="#FFF"
                                    message=""
                                    backgroundColor="#000"
                                  />
                          ...
                          className="flex h-[35px] w-[35px] cursor-pointer items-center justify-center rounded-md bg-[#C0FDB8] md:w-[70px]"
                        >
                          {loadingIcons['fez-delivery'] ? (
                            <Spinner className="border-[#049629]" />
                          ) : (
                            <Icon icon="ic:outline-save" className="text-[16px] text-[#049629]" />
                          )}
          ...
          {hasStoreMethods && (
            <div className="flex">
              <Link
                ...
                className="ml-auto mt-5 h-[42px] !w-[180] cursor-pointer rounded-lg bg-black px-3 py-2 text-white hover:text-white"
              >
