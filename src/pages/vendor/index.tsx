...
const Vendor = () => {
  ...
  const countryData = country?.data?.find((country: {id: any}) => country.id == isAuthenticatedUser?.store?.country_id)
  // append a route to the url
  useEffect(() => {
    if (type) {
      router.push(`/vendor?type=${type}`)
    }
  }, [type])
                ...
                <div className="flex flex-col items-center justify-center">
                  ...
                  <div className="flex items-center gap-1">
                    ...
                    <TextComponent as="p" className="text-[12px] font-medium leading-[24px] text-white">
                      ...
                      {isAuthenticatedUser?.store?.state} {countryData?.name}
                    </TextComponent>
                  </div>
                </div>
