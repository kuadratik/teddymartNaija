    ...
    try {
      ...
      if (data.success) {
        setCombinedMessages(prevMessages => {
          // Only include previous messages from the current chat
          const currentChatMessages = prevMessages.filter(msg => msg.chat_id === currentChatId)
          ...
          // Create a Map to maintain message order and handle duplicates
          const messageMap = new Map()

          // Add existing messages from current chat to map
          currentChatMessages.forEach(msg => messageMap.set(msg.id, msg))

          // Add new messages, overwriting any duplicates
          newMessages.forEach((msg: any) => messageMap.set(msg.id, msg))

          // Convert map back to array and sort by timestamp
          const combinedArray = Array.from(messageMap.values())
    ...
    <>
      ...
      <div className="mt-5 flex w-full flex-col items-center justify-center lg:mt-0 lg:min-h-[80vh]">
        ...
        <div className="flex w-full max-w-7xl gap-8 px-[5px] lg:mx-auto lg:px-[40px] xl:px-[0px]">
          ...
          <div className="min-h-[620px] w-full rounded-lg border shadow-f1 lg:min-h-[520px] lg:w-[350px] lg:border-[#E5E5EA] lg:bg-white">
