...
const SuccessCheckoutPage = () => {
  ...
  console.log('🚀 ~ SuccessCheckoutPage ~ paymentSuccessCheckoutData:', paymentSuccessCheckoutData[0]?.payment_status)
  // paymentSuccessCheckoutData[0]?.payment_status === 'pending_payment'
  //   ? 'pending_payment'
  //   : paymentSuccessCheckoutData[0]?.payment_status === 'failed_payment'
  //     ? 'failed_payment'
  //     : paymentSuccessCheckoutData[0]?.payment_status === 'completed_payment' || paymentSuccessCheckoutData[0]?.payment_status === 'approved_payment'
  //       ? 'completed_payment'
  //       : ''
  ...
  return (
    <div className="px-[20px] lg:px-0">
      {isVerifyPaymentLoading ? (
        ...
        <React.Fragment>
          {!paymentSuccessCheckoutData?.length ? (
            <EmptyClip />
          ) : (
                                        ...
                                        <div className="h-[60px] min-w-[60px] overflow-hidden rounded-[9px]">
                                          <Image
                                            src={`${process.env.imageBaseUrl}/${
                                              item?.variant_id && item?.listing?.variants?.length
                                                ? item?.listing?.variants.find((v: any) => v.id === item.variant_id)
                                                    ?.images?.[0] || item?.listing.images[0]
