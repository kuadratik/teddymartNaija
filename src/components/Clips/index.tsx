...
const getProductIdentifier = (product: any) => {
  // For variant products, include the variant ID in the identifier
  if (product?.variant?.id) {
    return `${product.slug}_variant_${product.variant.id}`
  }
  // For main products, just use the slug
  return product.slug
}
...
const ClipsComponent = () => {
  ...
  // Update this to store product identifiers instead of just slugs
  const [wishlistLoadingProducts, setWishlistLoadingProducts] = useState<string[]>([])

  // Modify to store product identifiers for both main products and variants
  ...
  const handleAddToWishlist = async (product: any) => {
    ...
    // Add product identifier to loading list
    setWishlistLoadingProducts(prev => [...prev, productId])
    try {
      await handleAddToWishListCart(product.slug, {
        variant_id: product?.variant?.id || undefined
      })
    } catch (error) {
      // ignore errors (e.g. already in wishlist) so it does not remove from cart
    } finally {
      // always remove from loading list
      setWishlistLoadingProducts(prev => prev.filter(id => id !== productId))
    }
  }

  // Create a function to check if a specific product is in loading state
  const isProductWishlistLoading = (product: any) => {
    const productId = getProductIdentifier(product)
    return wishlistLoadingProducts.includes(productId)
  }
  ...
  // For the quantity change issue, we'll add error handling
  const handleSafeQuantityChange = (product: any, newQuantity: number) => {
    if (newQuantity <= 0) {
      // Show delete confirmation if trying to reduce below 1
    ...
    // Use the product identifier for editing state
    const productId = getProductIdentifier(product)
    ...
    handleEditProduct({
      ...
      // Clear editing state in case of error
          ...
          <div className="mt-[10px] w-full space-y-4 lg:flex lg:gap-4">
                  ...
                  <tbody>
                    {allClipsIsSuccess &&
                      clipProductInfo.map((product: any, id: any) => {
                        console.log('🚀 ~ clipProductInfo.map ~ product:', product)
                        return (
                          <tr key={id} className="border-b">
                            <td className="px-4 py-4">
                              ...
                              >
                                <div className="h-[60px] min-w-[60px] overflow-hidden rounded-[9px]">
                                  <ImageComponent
                                    src={`${process.env.imageBaseUrl}/${
                                      product?.variant ? product?.variant?.image[0] : product?.images[0]
                                    }`}
                                    alt="store-image"
                                ...
                                <div>
                                  ...
                                  {product.variant && (
                                    <p className="text-xs text-gray-600">
                                      Variant: <span className="font-semibold">{product.variant.name}</span>
                                    </p>
                                  )}
                            ...
                            <td className="px-4 py-4">
                              <TextComponent as="p" className="text-[13px] leading-[16px] text-[#6B7280]">
                                <FormatNumberCurrency
                                  value={
                                    product.variant
                                      ? Number(product.variant.display_price) > 0
                                        ? product.variant.display_price
                            ...
                            <td className="px-4 py-4">
                              <div className="flex items-center space-x-2">
                                <button
                                  ...
                                  onClick={() => {
                                    handleSafeQuantityChange(product, product.quantity - 1)
                                  }}
                                >
                                ...
                                <span className="mx-2 text-[10px] leading-[12px]">
                                  {isEditProductLoading && editingProductId === getProductIdentifier(product) ? (
                                ...
                                <TextComponent as="p" className="text-[13px] leading-[16px] text-[#6B7280]">
                                  <FormatNumberCurrency
                                    value={
                                      product.variant
                                        ? Number(
                                            product.variant.display_price > 0
                                              ? product.variant.display_price
                                              : product.variant.price
                            ...
                            <td>
                              <div className="flex items-center gap-2 space-x-2">
                                <CustomButton
                                  className="flex w-fit items-center rounded-[8px] bg-[#AF52DE] py-2.5 hover:!bg-[#AF52De]"
                                  onClick={() => {
                                      ...
                                      showPlannerToast({
                                        options: {
                                          customToast: (
                                            <CustomToast
                                              altText={''}
                                              title={<>{'Please login to add this item to your wishlist!'}</>}
                                              textColor="#FFF"
                                  ...
                                  disabled={isProductWishlistLoading(product)}
                                >
                                  {isProductWishlistLoading(product) ? (
                                    <Spinner className="" />
                                  ) : (
                  ...
                  clipProductInfo.map((product: any, id: any) => (
                    <div key={id} className="flex items-center space-x-4 border-b px-[20px] pb-4 lg:my-0 lg:px-0">
                      ...
                      <div className="flex-1">
                        ...
                        {product.variant && (
                          <TextComponent as="p" className="text-xs text-gray-600">
                      ...
                      <div className="flex h-full flex-col justify-between gap-2">
                        ...
                        <CustomButton
                          className="flex items-center justify-center border bg-gray-100 py-1.5 text-[16px] shadow-f2"
                          onClick={() => {
                            if (isAuthenticatedToken) {
                              handleAddToWishlist(product)
                            } else {
                        ...
                        >
                          {isProductWishlistLoading(product) ? (
                            <Spinner className="border-black text-black" />
                          ) : (
            ...
            <div className="mt-6 flex w-full flex-col gap-[28px] bg-[#F0F1F5] p-5 lg:w-[40%] lg:rounded-[11px]">
              ...
              <CustomButton
                ...
                onClick={() => {
                  if (!isAuthenticated) {
                    router.push(`/auth/sign-up?redirect=${encodeURIComponent(`/clips/${data?.data?.cart_id}`)}`)
                    return
                  } else {
        ...
        >
          ...
          <div className="flex w-full flex-col gap-5">
            ...
            <div className="flex w-full items-center gap-4">
              ...
              <CustomButton
                onClick={
                  () => handleDeleteProduct(clipProduct?.slug, clipProduct?.variant?.id)
                  // if (clearSelection) {
