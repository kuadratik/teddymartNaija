...
const VendorNewLayout: React.FC<{children: React.ReactNode} & CustomerLayoutInterface> = ({children, maxWidth}) => {
  ...
  const MenuItems = React.useMemo(
    () => [
      ...
      {
        id: 2,
        link: `/vendor/${isActiveUser?.type === 'product' ? 'add-product' : 'add-service'}`,
  ...
  useEffect(() => {
    if (isAuthenticatedUser?.store === undefined) {
      router.push('/')
    }
  }, [isAuthenticatedUser?.store])
  ...
  // console.log(storepdown, 'storedropdown')
  ...
  return (
    <React.Fragment>
      <Layout style={{minHeight: '100vh', display: 'flex', flexDirection: 'column'}}>
        ...
        <Content
          style={{flex: 1, marginTop: maxWidth ? '' : '6px'}}
          className={`bg-transparent ${maxWidth ? '' : 'px-[10px] py-0 md:px-[85px]'}`}
        >
