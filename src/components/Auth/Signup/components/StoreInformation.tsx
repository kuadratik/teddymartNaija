...
const StoreInformation = (props: VendorOnboardingProps) => {
  const {handleChange, title_header, values, touched, errors, setFieldValue, setSelectedPaymentCountry} = props

  // Get user type to determine which categories to fetch
  const isActiveUser = useAppSelector(state => state.auth.activeUser)
  const userType =
    values.offers_product && values.offers_service
      ? 'both'
          ...
          <div className="w-full">
            <div className={`pb-1`}>
              ...
              <CountryInput
                ...
                disabled={false}
                setSelectedCountry={setSelectedPaymentCountry}
                errorMessage={errors.country ? errors.country : ''}
                value={values?.country ?? undefined}
                onChange={value => {
                  console.log('🚀 ~ StoreInformation ~ value:', value)
          ...
          <div className="w-full">
            <div className={`pb-1`}>
              <Tooltip
                title={
                  <p className="font-[500] text-black">
                    The selected state/province determines your store's currency and cannot be changed later. You can
                    choose a different state/province when creating a new store.
                  </p>
                }
                placement="topLeft"
                color="white"
