...
const LandingPage = () => {
  ...
  const categoryMap = useMemo(() => {
    return [
      {id: 10, title: 'Groceries & Gourmet Food'},
      {id: 11, title: 'Groceries & Gourmet Food'},
      {id: 2, title: 'Fashion, Beauty & Health'},
      {id: 3, title: 'Fashion, Beauty & Health'},
      {id: 5, title: 'Fashion, Beauty & Health'},
      {id: 4, title: 'Home & Kitchen'},
      {id: 1, title: 'Electronics, Books & Toys'},
      {id: 7, title: 'Electronics, Books & Toys'},
      {id: 8, title: 'Electronics, Books & Toys'},
      {id: 6, title: 'Sports & Outdoors'},
      {id: 9, title: 'Babies & Kids'},
      {id: 19, title: 'Construction, Tools & Hardware'},
      {id: 21, title: 'Construction, Tools & Hardware'}
    ]
  }, [])
  ...
  return (
    <MainWrapper className="">
          ...
          <div className="mx-auto w-full max-w-screen-xl lg:px-5 xl:px-0 2xl:max-w-screen-2xl">
            ...
            <div className="mx-auto max-w-7xl">
              {selectedOnclickCountry || selectedOnClickState ? (
                <div className="relative px-[35px] lg:top-6 lg:px-0">
        ...
        {formattedCategoryIds?.length! > 0 ? (
          ...
          <div className="flex flex-col">
            <MemoizedCategorySection
              category_id={[11, 12]}
              seeAllLinkHref="/category/11-12-groceries-food-dinning?type=product"
              showSeeAllLink={true}
            ...
            <MemoizedCategorySection
              category_id={[2, 3, 5]}
              seeAllLinkHref="/category/2-3-5-fashion-beauty-health?type=product"
              showSeeAllLink={true}
              formattedCategoryIds={formattedCategoryIds}
              selectedOnclickCountry={selectedOnclickCountry}
              type={type}
            ...
            <MemoizedCategorySection
              category_id={[4]}
              seeAllLinkHref="/category/4-home-kitchen?type=product"
              showSeeAllLink={true}
              formattedCategoryIds={formattedCategoryIds}
              selectedOnclickCountry={selectedOnclickCountry}
              type={type}
            ...
            <MemoizedCategorySection
              category_id={[1, 7, 8]}
              seeAllLinkHref="/category/1-7-8-electronics-books-toys?type=product"
              showSeeAllLink={true}
              formattedCategoryIds={formattedCategoryIds}
              selectedOnclickCountry={selectedOnclickCountry}
              type={type}
            ...
            <MemoizedCategorySection
              category_id={[6]}
              seeAllLinkHref="/category/6-sports-outdoors?type=product"
              showSeeAllLink={true}
              formattedCategoryIds={formattedCategoryIds}
              selectedOnclickCountry={selectedOnclickCountry}
              type={type}
            ...
            <MemoizedCategorySection
              category_id={[9]}
              seeAllLinkHref="/category/9-babies-kids?type=product"
              showSeeAllLink={true}
              formattedCategoryIds={formattedCategoryIds}
              selectedOnclickCountry={selectedOnclickCountry}
              type={type}
            ...
            <MemoizedCategorySection
              category_id={[19, 21]}
              seeAllLinkHref="/category/19-construction-tools-hardware?type=product"
              showSeeAllLink={true}
              formattedCategoryIds={formattedCategoryIds}
              selectedOnclickCountry={selectedOnclickCountry}
              type={type}
      ...
      <ScrollOptimizer
        categories={[10, 11, 2, 3, 5, 4, 1, 7, 8, 6, 9, 19, 21]}
        selectedLanguage={selectedLanguage}
        type={type}
...
const ScrollOptimizer = memo(({categories, selectedLanguage, type, categoryMap}: any) => {
  const dispatch = useDispatch<AppDispatch>()
  // Start with 0 instead of -1 to load first section immediately
  const [visibleSection, setVisibleSection] = useState(0)
  console.log('🚀 ~ ScrollOptimizer ~ visibleSection:', visibleSection)
  const [hasPrefetched, setHasPrefetched] = useState<Record<number, boolean>>({})
  const scrollTimerRef = useRef<NodeJS.Timeout | null>(null)
  const initialLoadRef = useRef(false)

  // Function to find which category sections are currently in the viewport
  const findVisibleSections = useCallback(() => {
    // Get all category sections by their class or id
    const sections = document.querySelectorAll('[data-category-section]')
    if (!sections.length) return 0 // Default to first section if none found

    const viewportHeight = window.innerHeight
    const buffer = viewportHeight * 0.3 // 30% of viewport as buffer

    // First, check if the first section is still in view
    const firstSection = sections[0]
    const firstRect = firstSection.getBoundingClientRect()
    if (firstRect.top > -buffer && firstRect.bottom > 0) {
      return 0 // Keep first section active when it's still partially visible
    }

    // Find the first section that's in or approaching the viewport
    for (let i = 0; i < sections.length; i++) {
      const section = sections[i]
      const rect = section.getBoundingClientRect()

      // Check if section is in viewport or approaching (within buffer)
      if (rect.top < viewportHeight + buffer && rect.bottom > 0 - buffer) {
        // Return the index of the first visible section
        return i
      }
    }
  ...
  // Immediate prefetch of first section on component mount
  useEffect(() => {
    if (!initialLoadRef.current && categories.length > 0) {
      // Prefetch first category immediately
      const firstCategoryId = categories[0]
      prefetchCategory(firstCategoryId, 0)
  ...
  useEffect(() => {
    const handleScroll = () => {
      ...
      // Set a new timer for 100ms debounce
      scrollTimerRef.current = setTimeout(() => {
        const currentVisibleSection = findVisibleSections()
        if (currentVisibleSection !== visibleSection) {
    ...
    // Initial check after components have rendered
    setTimeout(() => {
      handleScroll()
    }, 300)
  ...
  }, [visibleSection, findVisibleSections])

  // Smart prefetching based on visible section and scroll direction
  useEffect(() => {
    if (!StoreEndpoint.util || !selectedLanguage?.value) return

    // Calculate which sections need prefetching (next section and one after that)
    const sectionsToFetch = [visibleSection + 1, visibleSection + 2]
      .filter(index => index < categories.length) // Only valid indices
      .filter(index => !hasPrefetched[index]) // Only fetch if not already fetched
    ...
    sectionsToFetch.forEach((sectionIndex, i) => {
      const categoryIndex = Math.min(sectionIndex, categories.length - 1)
      const categoryId = categories[categoryIndex]

      // Staggered timeout to avoid concurrent requests
      setTimeout(() => {
        prefetchCategory(categoryId, sectionIndex)
      }, i * 500) // Stagger by 500ms
    })
  }, [visibleSection, categories, prefetchCategory])
