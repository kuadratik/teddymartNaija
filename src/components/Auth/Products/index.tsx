...
const LandingPage = () => {
  ...
  }, [formattedCategoryIds, selectedLanguage?.value, selectedOnclickCountry?.id, type])

  const {currentData: data, isLoading, isFetching} = useGetSearchStoreListingQuery(searchStoreArgs ?? skipToken)
  ...
  useEffect(() => {
    ...
    if (!StoreEndpoint.util || !selectedLanguage?.value) {
      return clearPendingPrefetch
    }

    const cacheKey = `${selectedLanguage.value}-${type}`
    if (PREFETCHED_CATEGORY_KEYS.has(cacheKey)) {
      return clearPendingPrefetch
    }

    PREFETCHED_CATEGORY_KEYS.add(cacheKey)
  ...
  return (
    <MainWrapper className="">
      ...
      <div className="flex w-full max-w-7xl flex-col lg:mx-auto">
        {formattedCategoryIds && formattedCategoryIds.length > 0 ? (
          ...
          <div className="flex flex-col max-w-7xl gap-10 pb-10">
...
const ScrollOptimizer = memo(({categories, selectedLanguage, type, categoryMap}: any) => {
  ...
  const hasPrefetchedRef = useRef<Record<number, boolean>>({})
  const scrollTimerRef = useRef<ReturnType<typeof setTimeout> | null>(null)
  const initialLoadRef = useRef(false)
  const intersectionObserverRef = useRef<IntersectionObserver | null>(null)
  const pendingPrefetchTimersRef = useRef<Array<ReturnType<typeof setTimeout>>>([])

  // Function to prefetch a specific category with better error handling
  const prefetchCategory = useCallback(
    (categoryId: number, sectionIndex: number) => {
      if (!StoreEndpoint.util || !selectedLanguage?.value || hasPrefetchedRef.current[sectionIndex]) return
      ...
      try {
        ...
        hasPrefetchedRef.current[sectionIndex] = true
      } catch (error) {
  ...
  useEffect(() => {
          ...
          if (visibleEntry) {
            ...
            if (visibleSectionRef.current !== sectionIndex) {
              ...
              const sectionsToFetch = [sectionIndex + 1, sectionIndex + 2]
                ...
                .filter(index => !hasPrefetchedRef.current[index])

              sectionsToFetch.forEach((sectionToFetch, i) => {
                const categoryId = categories[Math.min(sectionToFetch, categories.length - 1)]
                const timerId = setTimeout(() => {
                  prefetchCategory(categoryId, sectionToFetch)
                }, i * 300)

                pendingPrefetchTimersRef.current.push(timerId)
              })
            }
          }
        },
      ...
      const handleScroll = () => {
        ...
        scrollTimerRef.current = setTimeout(() => {
          ...
          if (visibleSectionRef.current !== currentSection) {
            ...
            if (nextSection < categories.length && !hasPrefetchedRef.current[nextSection]) {
    ...
    // Cleanup function
    return () => {
      pendingPrefetchTimersRef.current.forEach(timerId => clearTimeout(timerId))
      pendingPrefetchTimersRef.current = []
      if (scrollTimerRef.current) {
        clearTimeout(scrollTimerRef.current)
        scrollTimerRef.current = null
      }
...
const TopDealComponent = () => {
  ...
  return (
    <div className="mb-2 h-[400px] w-full rounded-[8px]">
