    ...
    <section className="w-full bg-[#DFE1E5] lg:mx-auto lg:overflow-x-scroll">
      {isDesktop && (
        <div className="flex w-full flex-col items-center gap-2 lg:h-[56px] lg:flex-row lg:justify-evenly lg:overflow-x-scroll">
          <div className="mx-auto grid h-full w-full max-w-screen-xl grid-cols-3 items-center justify-center gap-2 px-5 lg:px-[10%] xl:px-0 2xl:max-w-screen-2xl">
            {navigationItems.map((item, i: number) => {
              return item?.name.toLowerCase() === 'find a vendor' ? (
                ...
                <button
                  className={`flex h-[42px] w-full cursor-pointer flex-row items-center justify-center gap-2 rounded-[8px] px-[36px] py-[9px] hover:bg-black hover:text-white ${router.pathname === item.path ? 'bg-black text-white' : 'text-black'} `}
                  key={i}
                  onClick={() => {
                    if (item?.path !== '') {
                ...
                >
                  <p className="relative whitespace-nowrap text-center text-[14px] font-medium leading-[18px]">
                    {item?.name}{' '}
                    {item?.name.toLowerCase() === 'messages' && ReceivedMessage?.length > 0 && (
                      <Icon
                        ...
                        width="34"
                        height="34"
                        className="absolute -right-6 -top-3 animate-pulse text-red-600"
                      />
                    )}
                  </p>
                </button>
