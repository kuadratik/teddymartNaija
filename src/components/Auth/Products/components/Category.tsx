...
const Category = ({open, setOpen}: ICategoryProps) => {
  ...
  return (
    <section className="flex w-full touch-pan-x snap-x snap-mandatory overflow-x-scroll lg:mx-auto">
      {/* {isDesktop && ( */}
      <div className="flex w-full flex-row items-center gap-4 px-[10px] lg:px-[24px]">
        {AllCategories?.map((category: CategoriesProps, i: number) => (
          ...
          >
            ...
            <p className="whitespace-nowrap text-center text-[12px]">{category?.name}</p>
          </div>
        ))}
      </div>
      {/* )} */}
            ...
            {AllCategories?.map((category: CategoriesProps, i: number) => (
              <SwiperSlide className="w-full" key={i}>
                <div
                  ...
                  onClick={() => {
                    setSelectedId(category.id ?? null)
                    router.push(`/category/${category.id}-${category.slug}?type=${category.type}`)
                  }}
                >
                  <div
                    style={{backgroundColor: category.filledColor}}
