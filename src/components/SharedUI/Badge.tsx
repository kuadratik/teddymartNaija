...
function Badge({status, className, textClassName}: Props) {
  ...
  return (
    <p
      style={{backgroundColor: bg, color: text, display: 'flex', alignItems: 'center', justifyContent: 'center'}}
      className={cx(
        '!flex w-full min-w-[120px] max-w-[120px] !items-center !justify-center rounded-[4px] bg-opacity-30 p-1 px-6 text-center text-[14px] font-normal',
        className
      )}
    >
      <span className={cx('', textClassName)}>{status == 'Outofstock' ? 'Out of stock' : status}</span>
    </p>
  )
}
