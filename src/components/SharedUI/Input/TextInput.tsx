...
const TextInput = ({
  ...
  return (
    <div className="w-full">
      {!title || title === '' ? null : (
        <div className={`pb-1`}>
          <label htmlFor={id} className={twMerge('text-sm font-[500] capitalize text-[#6B7280]', labelClassName)}>
            {title}
          </label>
        </div>
      )}
      <div className="relative">
        <input
          ...
          multiple={multiple}
          className={twMerge(
            `w-full rounded-[8px] border ${errorMessage ? 'border-red-600' : 'border-gray-300'} bg-[#F9FAFB] px-[15px] py-[11px] text-[#141414] shadow-f1 placeholder:font-medium hover:border-[#000000] focus:outline-none focus:ring-1 focus:ring-[#000000]`,
            className
          )}
          accept={accept}
          disabled={disabled}
