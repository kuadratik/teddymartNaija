import {Button, Dropdown, Pagination, Rate, Space, Table, Tooltip} from 'antd'
...
const ProductTable = ({loading, data, isFetching, queryParams, updateQueryParams}: ProductTableProps) => {
  ...
  const columns: ColumnsType<any> = React.useMemo(() => {
    return [
      ...
      },
      {
        ...
        title: 'Price',
        dataIndex: 'price',
        render: (text, record) => <FormatNumberCurrency value={+record?.price} currency={isActiveUser?.currency} />,
        ...
        render: (text, record) => (
          <>
            {record?.quantity == 0 ? (
              ...
              <TextComponent as="p" className="font-normal !text-[#6b7280] lg:ml-8">
                  ...
                  label:
                    record?.quantity <= 3 ? (
                      ...
                      <Tooltip
                        title={
                          <p className="text-black">
                            {'You can only delete items with  a stock count of less than 3.'}
                          </p>
                        }
                        color="white"
                      >
  ...
  const locale = {
    emptyText: <EmptyResult showBtn onClick={() => {}} title="" text="Add product" />
  }
