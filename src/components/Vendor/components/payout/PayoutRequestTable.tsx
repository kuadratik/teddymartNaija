...
import {payoutInfo} from './PayoutCompleteTable'
...
export const StatusRenderer = ({text}: {text: string}) => {
  text = text == 'delivered' ? 'New' : text
  return (
    <TextComponent
      as="p"
      className={`!w-[113px] rounded-md py-[12px] text-center font-normal capitalize md:ml-0 ${text === 'Processing' ? 'bg-[#FFFAEA] text-[#FF9500]' : 'bg-[#E7F2FF] text-[#044BFD]'}`}
    >
      {text}
    </TextComponent>
  )
      ...
      },
      {
        ...
        render: (text: string, record: any) => (
          <TextComponent as="p" className="flex items-center text-black">
            <Icon icon={getIcon(record.currency)} />
            {record.total_amount}
          </TextComponent>
        )
      },
