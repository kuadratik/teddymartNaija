...
const MekDirectoryCard = ({business, defaultLogo, setIsLoadingImage}: IProps) => {
  console.log('🚀 ~ MekDirectoryCard ~ business:', business)
  ...
  const contactNumbers: MenuProps['items'] = [
    {
      ...
      label: (
        <div className="flex flex-col gap-2">
          <a target="_blank" rel="noopener noreferrer" href={`tel:${newFormatPhoneNumberByCountry(`+${business.business_contact_number ?? ''}`)}`}>
            {newFormatPhoneNumberByCountry(`+${business.business_contact_number ?? ''}`)}
          </a>
          {business.secondary_contact_number && business?.show_secondary_contact && (
            <a target="_blank" rel="noopener noreferrer" href={`tel:${newFormatPhoneNumberByCountry(`+${business.secondary_contact_number ?? ''}`)}`}>
              {newFormatPhoneNumberByCountry(`+${business.secondary_contact_number ?? ''}`)}
            </a>
          )}
        </div>
      )
  ...
  const emailLists: MenuProps['items'] = [
    {
      ...
      label: (
        <div className="flex flex-col gap-2">
          {business?.show_business_email && (
            <a target="_blank" rel="noopener noreferrer" href={`mailto:${business.business_email}`}>
              {business.business_email}
            </a>
          )}
          {business.secondary_business_email && business?.show_secondary_email && (
    ...
    },
    {
      ...
      label: (
        <button
          ...
          className="flex items-center gap-1 py-1 text-[13px] font-medium text-[#4D4D4D] lg:py-0"
        >
          <Icon icon="fluent:share-20-regular" width="18" height="16" /> Refer a business
        </button>
      )
    },
    {
    ...
    <>
      ...
      {isReferralModalOpen && (
        <ReferralModal
          ...
          referralLink={`${baseUrl}/auth/sign-up?redirect=%2Fget-list&referral_code=${(isAuthenticatedUser as any)?.referral_code}&referralType=business_owner`}
          formValues={''}
          title="Invite a Business"
          description="Refer a business owner to list on myEKI today!"
          setFormValues={() => {}}
