      ...
      <div className="relative flex h-full w-full flex-col">
        ...
        >
          {chatMessageIsLoading || (chatMessageIsFetching && hideIsFetching) ? (
            ...
            <InfiniteScroll
              ...
              loader={
                isLoadingMore &&
                combinedMessages.length > 14 && (
                  <div className="flex animate-pulse justify-center text-center italic text-gray-700">Loading...</div>
                )
              }
              endMessage={
                !hasMoreMessages &&
                combinedMessages.length > 20 && (
                  <div onClick={scrollToBottom} className="flex w-full justify-center py-2">
                    <p className="fixed-message mx-auto w-fit cursor-pointer rounded-full bg-black/70 px-5 py-2 text-center text-sm text-white">
                      No more messages
                    </p>
                  </div>
                )
        ...
        >
          <div className="flex w-full items-center justify-between">
            <TextAreaInput
              row={3}
              className={`${windowWidth < 1000 && formValues.message.length > 80 ? 'h-30' : 'h-10'} w-full rounded-none border-none focus:border-0 focus:ring-0 text-lg lg:text-base`}
