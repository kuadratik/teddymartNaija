          ...
          advertPlans?.map((plan: any, index) => (
            <div className="flex flex-col gap-1" key={plan.id}>
              <div className="rounded-[8px] bg-[#F9FAFB] px-2 py-4">
                <button
                  disabled={payment === null ? false : true}
                  className="flex cursor-pointer items-center gap-4"
                  onClick={() => onSelectPlan(plan.id)}
                >
                  <Checkbox disabled={payment === null ? false : true} checked={selectedPlanId === plan.id} />
                  <TextComponent
                    as="span"
                    className={`text-[13px] font-[500] leading-[16px] ${payment === null ? '' : 'opacity-60'}`}
                  >
                    {plan.price === '0.00' ? (
                      plan.name
                    ) : (
