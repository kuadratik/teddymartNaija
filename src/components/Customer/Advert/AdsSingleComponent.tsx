...
const AdsSingleComponent = ({data, className = '', isHideDeleteSaveAds = false, refetch}: IProps) => {
              ...
              <div className="relative flex flex-col px-2 py-1 lg:flex-row lg:items-center">
                ...
                <div className="flex flex-col justify-center p-4 lg:w-2/3">
                  ...
                  <p className="text-[14px] lg:h-[63px]">
                    {capitalizeOnlyFirstLetter(ad?.description).length > 60 ? (
                      <>
                        {capitalizeOnlyFirstLetter(ad?.description).slice(0, 60)}...{' '}
                        <span
                          onClick={() => {
                            router.push(`/ads-gallery/${ad.id}`)
                          }}
