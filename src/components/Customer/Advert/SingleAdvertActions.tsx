...
const SingleAdvertActions = ({data, adsRatedRefetch, isWishlisted, refetchSingleAdvert}: IProps) => {
  ...
  console.log("🚀 ~ SingleAdvertActions ~ isAuthenticatedUser:", isAuthenticatedUser)
  const [formRatingValues, setFormRatingValues] = useState({
    rating: 1,
    name: isAuthenticatedUser ? `${isAuthenticatedUser?.last_name} ${isAuthenticatedUser?.first_name} ` : '',
    review: ''
  })
