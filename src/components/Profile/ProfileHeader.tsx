      ...
      <div className="relative flex flex-col items-center justify-center">
        ...
        <div className="relative">
            ...
            <div className="relative flex h-[75px] w-[75px] -translate-y-9 items-center justify-center overflow-hidden rounded-full bg-[#fff] shadow">
              {isLoading || updateStoreIsLoading ? (
                ...
                <>
                  <Image
                    src={
                      isAuthenticatedUser?.store?.profile_picture_path
                        ? `${process.env.imageBaseUrl}/${isAuthenticatedUser?.store?.profile_picture_path}`
                    ...
                    alt="profile"
                    preview={false}
                    className={`-z-[9999px] !h-[70px] cursor-pointer !w-[70px] rounded-full object-cover ${isLoadingImage ? 'blur-sm' : ''}`}
                    width={70}
                    height={70}
                  />
                </>
                ...
                onChange={e => {
                  if (e.target.files) {
                    handleUpdateProfilePicture({
                      payload: {...isAuthenticatedUser?.store, profile_picture_path: e.target.files[0]}
                    })
                  }
                }}
              />
          ...
          <Icon
            ...
            color="black"
            height={20}
            className="absolute cursor-pointer left-[58px] top-[16px] !z-[9999px] text-3xl text-black"
          />{' '}
        </div>
      </div>
