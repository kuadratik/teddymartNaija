  ...
  return (
    <div className="rounded-[16px] border border-white bg-white/20 p-6 backdrop-blur-sm transition-shadow hover:shadow-md">
        ...
        <div className="flex items-center gap-3">
          ...
          <div className="flex flex-col">
            <div className="flex items-center gap-2">
              <TextComponent as="h3" className="truncate text-[18px] font-semibold leading-tight text-gray-900">
                {brand.name}
              </TextComponent>
            ...
            {brand.categories && brand.categories.length > 2 ? (
              ...
              >
                <span className="mt-1 cursor-pointer whitespace-nowrap text-[14px] font-normal text-gray-500">
                  {categoryText}
                </span>
              </Tooltip>
            ) : (
              <TextComponent as="p" className="mt-1 whitespace-nowrap text-[14px] font-normal text-gray-500">
                {categoryText}
              </TextComponent>
            )}
          </div>
      ...
      <div className="flex items-center justify-between gap-3 rounded-xl border border-white bg-white/20 p-3 backdrop-blur-sm transition-shadow">
        ...
        <button
          onClick={() => {
            const url = ('target_url' in brand && brand.target_url) || ('source_url' in brand && brand.source_url)
            if (url) {
