...
const ProductClipBox = ({data, selectedVariantId}: ProductClipBoxType) => {
  ...
  useEffect(() => {
    if (selectedVariantId && data?.variants) {
      ...
      if (selectedVariant) {
        // Merge the variant data with basic product data, including discount fields
        setCurrentItem({
          ...data,
          ...
          discount_start_date: selectedVariant.discount_start_date,
          discount_end_date: selectedVariant.discount_end_date,
      ...
      <div className="mt-2">
        <div className="flex justify-between text-base text-gray-500">
          ...
          <span className="font-semibold">
            {currentItem?.weight ? `${Number(currentItem?.weight).toLocaleString()}g` : 'Not Specified'}
          </span>
        </div>
      </div>
      {/* Amount section */}
            ...
            currentItem?.discount_start_date?.length ? (
              <span className="block text-gray-400 line-through">
                <FormatNumberCurrency
                  value={+parseFloat(currentItem?.display_price) + parseFloat(currentItem?.display_price) * 0.1}
            ...
            <span className="text-xl font-bold text-black">
              <FormatNumberCurrency
                value={
                  ...
                  dayjs(currentItem?.discount_end_date).isAfter(dayjs()) &&
                  currentItem?.discount_start_date?.length
                    ? currentItem?.display_price
                    : +currentItem?.display_price > 0
                      ? currentItem?.display_price
                      : currentItem?.price
                }
                currency={currentItem?.currency}
