...
import AlsoLikeComponent from '../Auth/Products/components/AlsoLike'
import CustomerProductDetailsView from './CustomerProductDetailsView'
import ProductClipBox from './ProductClipBox'
...
const NewProductInfo = ({isLoading, data, setParams}: IProps) => {
  ...
  const {data: storeInfo, isLoading: storeInfoIsLoading} = useGetSpecifiedStoreListingQuery(
    data?.data?.store_id && type ? {params: {listingType: type, store: data.data.store_id}} : skipToken
  )

  const [currVariant, setCurrVariant] = useState(null)
  console.log('🚀 ~ NewProductInfo ~ currVariant:', currVariant)
  ...
  console.log('🚀 ~ NewProductInfo ~ productData:', productData)

  // console.log('currentVariant', productData?.data?.variants[currVariant || 0])
  ...
  return (
    <div className="px-[20px] lg:px-0">
      ...
      <Content className="mt-[47px] rounded-md bg-white p-[18px] text-[#000]">
        <div className="flex flex-col gap-6 lg:flex-row">
          <div className="flex-shrink-0 lg:w-[400px]">
              ...
              >
                {(currVariant !== null ? productData?.data?.variants[currVariant] : productData?.data)?.images?.map(
                  (listing: any, i: number) => {
                    // console.log('currentImageUrl1', listing)
                    return (
            ...
            <div className="mt-[10px] grid grid-cols-3 gap-3 md:flex md:gap-2">
              {(currVariant !== null
                ? productData?.data?.variants[currVariant]?.images
                : productData?.data?.images
              )?.map((productIn: any, id: any) => {
                return (
                  <div key={id} className="flex cursor-pointer gap-2" onClick={() => goToSlide(id)}>
                    <div
                      className={`h-[86px] min-w-[94px] overflow-hidden rounded-[8px] ${
                        activeIndex === id ? 'border-[2px] border-black' : 'border-[1px] border-[#F2F2F2]'
                      }`}
                    >
                      <ImageComponent
                        isLoadingImage={isLoadingImage}
          ...
          <ProductClipBox data={productData?.data} selectedVariantId={selectedVariantId!} />
        </div>
        ...
        <div>
          <AlsoLikeComponent titleText="You may also like" />
        </div>
      </Content>
    </div>
  )
