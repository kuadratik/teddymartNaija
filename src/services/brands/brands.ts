...
export const BrandsEndpoint = apiWithTag.injectEndpoints({
  ...
  endpoints: build => ({
    ...
    getBrandBySlug: build.query<BrandBySlugResponse, string>({
      query: slug => ({
        url: `/front/brand/${slug}`,
        method: 'GET'
      }),
      providesTags: ['brandsApi']
    })
...
export const {useGetBrandsQuery, useGetBrandCategoriesQuery, useGetBrandBySlugQuery} = BrandsEndpoint
