...
export const StoreEndpoint = apiWithTag.injectEndpoints({
  endpoints: build => ({
    ...
    >({
      query: ({listType, category, search, uuid, currency, sortType, page, per_page}) => {
        const params = new URLSearchParams()
        params.append('listingType', listType)
    ...
    getUserStore: build.query<IUserStoreResponse, {currency?: string}>({
      query: ({currency}) => {
        return {
