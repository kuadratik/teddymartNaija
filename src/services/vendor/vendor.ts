...
const apiWithTag = api.enhanceEndpoints({
  addTagTypes: ['vendorApi', 'store-listing-total']
})
...
export const VendorEndpoint = apiWithTag.injectEndpoints({
  endpoints: build => ({
    ...
    >({
      query: arg => {
        const {from, to, page, search, listingType = 'product', availability = true} = arg
    ...
    createUserStoreListingItem: build.mutation<any, {body: any}>({
      query: ({body}) => ({
        url: `/store/listings/create`,
        method: 'POST',
        body
      }),
    ...
    updateUserStoreItem: build.mutation<any, {userStore: string; listing: string; body: any}>({
      query: ({userStore, listing, body}) => ({
