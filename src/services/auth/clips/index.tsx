...
export const ClipsEndpoint = apiWithTag.injectEndpoints({
  endpoints: build => ({
    ...
    getUserOrder: build.query<
      ...
      {order_status?: 'incart' | 'pending' | 'complete'; currency: string}
    >({
      query: ({order_status, currency}) => ({
        url: `front/order`,
        method: 'GET',
        params: {
    ...
    getUserOrderDetails: build.query<IUserTransactionHistoryResponse, {order_id: string}>({
      query: ({order_id}) => ({
        url: `front/${order_id}/order`,
        method: 'GET'
      }),
      providesTags: ['all-clips']
    }),
